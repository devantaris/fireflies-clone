  ...
  return (
    <div className="min-h-full flex flex-col bg-[var(--bg)]">
      ...
      <div className="border-b border-[var(--border)] px-6 py-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-base font-semibold text-[var(--text-1)]">AskFred</h1>
          <p className="text-xs text-[var(--text-3)]">AI assistant for your meetings</p>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8 max-w-lg">
          {SUGGESTIONS.map((s) => (
            <button
              key={s}
              onClick={() => setInput(s)}
              className="px-3.5 py-2 rounded-full border border-[var(--border-strong)] bg-[var(--bg-card)] text-xs text-[var(--text-2)] hover:border-[#6c47ff]/50 hover:text-[var(--text-1)] hover:bg-[#1a1040]/30 transition-all"
            >
              {s}
            </button>
          ))}
      ...
      <div className="border-t border-[var(--border)] px-6 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3 bg-[var(--bg-card)] border border-[var(--border-strong)] rounded-xl px-4 py-3 focus-within:border-[#6c47ff]/50 transition-colors">
          <input
            value={input}
