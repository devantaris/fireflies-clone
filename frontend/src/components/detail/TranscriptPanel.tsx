    ...
    <div
      ...
      className={`group flex gap-3 px-4 py-3 cursor-pointer transition-colors rounded-lg mx-2 ${
        isActive
          ...
          : isMatch
          ? "bg-yellow-400/5 hover:bg-[var(--bg-hover)]"
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`text-xs font-semibold ${
              isActive ? "text-[var(--accent-text)]" : "text-[var(--text-2)]"
            }`}
          >
            {line.speaker}
          </span>
          <button
            ...
            className={`text-[11px] tabular-nums transition-colors ${
              isActive
                ...
                : "text-[var(--text-4)] hover:text-[#6c47ff]"
            }`}
          >
        ...
        <p
          className={`text-sm leading-relaxed ${
            isActive ? "text-[var(--text-1)]" : "text-[#a0a0a0]"
          }`}
        >
  ...
  if (lines.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
        <div className="w-12 h-12 rounded-xl bg-[var(--bg-elevated)] border border-[var(--border-strong)] flex items-center justify-center mb-3">
          <Maximize2 size={20} className="text-[var(--text-4)]" />
        </div>
        <p className="text-sm font-medium text-[var(--text-2)] mb-1">No transcript</p>
      ...
      <div className="shrink-0 px-4 py-2.5 border-b border-[var(--border)] flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-[var(--bg-sub)] border border-[var(--border-strong)] rounded-lg pl-8 pr-8 py-1.5 text-xs text-[var(--text-1)] placeholder-[var(--text-4)] focus:outline-none focus:border-[#6c47ff] transition-colors"
          />
          {searchQuery && (
            <button
              onClick={clearSearch}
            ...
            {matchIds.length > 0 && (
              <>
                <button
                  ...
                  className="p-1 rounded text-[var(--text-3)] hover:text-[var(--text-1)] hover:bg-[var(--border)] transition-colors"
                >
                  <ChevronUp size={14} />
                </button>
                <button
        ...
        {!searchQuery && (
          <button
            ...
            className={`shrink-0 text-[11px] px-2 py-1 rounded transition-colors ${
              followPlayback
                ? "text-[var(--accent-text)] bg-[#6c47ff]/10"
